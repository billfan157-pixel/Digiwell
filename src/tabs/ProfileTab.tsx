...
export default function ProfileTab({
  ...
  const recalculateLevel = async () => {
    if (!profile?.id) return;

    try {
      const { data, error } = await supabase.rpc('recalculate_user_level', {
        user_id: profile.id
      });
          ...
          <div className="flex-1 min-w-0">
            <div className="min-w-0">
              <div className="flex items-center gap-2 min-w-0">
                ...
                {isPremium && streakFreezes !== undefined && <div className={`px-2 py-1 rounded-lg border flex items-center gap-1 cursor-pointer transition-all ${needsFreeze ? 'bg-red-500/20 border-red-500/40 text-red-400 hover:bg-red-500/30 animate-pulse' : 'bg-blue-500/10 border-blue-500/20 text-blue-400 hover:bg-blue-500/20'}`} title={needsFreeze ? 'Dùng Streak Freeze để bảo vệ chuỗi!' : `Streak Freeze: ${streakFreezes} lần/tháng`} onClick={needsFreeze ? () => useStreakFreeze?.().then((success: boolean) => {
